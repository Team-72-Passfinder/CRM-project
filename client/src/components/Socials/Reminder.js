...
import { getContactsForReminder } from '../../api';
    ...
    <Dialog onClose={handleClose} open={!close}>
      <DialogTitle>
        ...
        <Typography sx={{ fontSize: '14px', fontWeight: 400 }} align="left">
          Hey, it appears you haven't had any events or been in touch with them
          for a while!
        </Typography>
      </DialogTitle>
        ...
        <List>
          {uncontact.length !== 0 &&
            uncontact.names.map((name, i) => (
              <ListItem
                key={uncontact.contactIds[i]}
                button
                onClick={(e) => handleClick(uncontact.contactIds[i])}
              >
                <ListItemAvatar>
